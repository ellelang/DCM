...
dataset$lake1000 <- dataset$implakes/1000
colnames(dataset)
betas <- read.table("betas04162020.txt", sep = "\t", header = TRUE)
dim(betas)
varcov <- read.table("varscov04162020.txt", sep = "\t", header = FALSE)
dim(varcov)
...
for (i in 1:s){
  ...
  for (r in 1 : R){
    ...
    new_beta2<- c(0,0,0,0,newbetas[5:13],0,newbetas[15:19],0)
    index_i <- index [i]
    dat_s <- dat0[i,]
...
NM_CIL
NM_CIU
write.csv (x = WLD_WTA_ALL, file = "WLD_wta_0416.csv", row.names = FALSE)
write.csv (x = CC_WTA_ALL, file = "CC_wta_0416.csv", row.names = FALSE)
write.csv (x = NM_WTA_ALL, file = "NM_wta_0416.csv", row.names = FALSE)
