...
dataset$lake1000 <- dataset$implakes/1000
colnames(dataset)
betas <- read.table("betas04222020.txt", sep = "\t", header = TRUE)
dim(betas)
varcov <- read.table("varscov04222020.txt", sep = "\t", header = FALSE)
dim(varcov)
...
varcov_m <- as.matrix(varcov, nrow = 23, ncol = 23)
...
dat0 <- select (dataset, Wetland, Payment, Covercrop, NuMgt, asc, 
                ...
                areaf_2, areaf_3, areaf_4, aware, past, appreciate,resp, info_so,concern,lake1000,lake1000)

dat0['plake'] = dataset$lake1000
dim(dat0)
############INDIVIDUAL WTA estimate EXAMPLE
draws <- matrix (NA,nrow = 10, ncol = 23)
colnames(draws)<-c('wetland','pay','cc','nm','asc','dfl','costtax',
                   'incfarm1','incfarm23','incfarm4','incfarm5','incfarm6',
                   'farmsi2','farmsi3','farmsi4','fscaware','fscpast','fscapp','fsresp','fsinfo','fsconcern','plake','clake')
dim(varcov_m)
for (r1 in 1 : 10){
  drawbetas<- betas
...
for (i in 1:s){
  ...
  draws <- matrix (NA,nrow = R, ncol = 23)
  colnames(draws)<-c('wetland','pay','cc','nm','asc','dfl','costtax',
                     'incfarm1','incfarm23','incfarm4','incfarm5','incfarm6',
                     'farmsi2','farmsi3','farmsi4','fscaware','fscpast','fscapp','fsresp','fsinfo','fsconcern','plake','clake')
  set.seed(4008)
  ...
  for (r in 1 : R){
    ...
    new_beta2<- c(0,0,0,0,newbetas[5:13],0,newbetas[15],0,newbetas[17:18],0,newbetas[20:21],0,0)
    index_i <- index [i]
    dat_s <- dat0[i,]
    dat_s_m <- as.matrix(x = dat_s, nrow = 1, ncol = 23)  
...
NM_CIL
NM_CIU
write.csv (x = WLD_WTA_ALL, file = "WLD_wta_0422.csv", row.names = FALSE)
write.csv (x = CC_WTA_ALL, file = "CC_wta_0422.csv", row.names = FALSE)
write.csv (x = NM_WTA_ALL, file = "NM_wta_0422.csv", row.names = FALSE)
