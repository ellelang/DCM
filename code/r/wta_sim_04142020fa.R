...
dat0 <- select (dataset, Wetland, Payment, Covercrop, NuMgt, asc, 
                ...
                areaf_2, areaf_3, areaf_4, aware, past, app,social,lake1000)
...
colnames(draws)<-c('wetland','pay','cc','nm','asc','dfl','costtax',
                   ...
                   'farmsi2','farmsi3','farmsi4','fscaware','fscpast','fscapp','fsresp','clake')
dim(varcov_m)
for (r1 in 1 : 10){
  drawbetas<- betas
...
clusterinfo = read.csv('merged_cluster_0504.csv')
id = clusterinfo$id
cluster= clusterinfo$Cluster

WLD_WTA_ALL

#wld = wld %>% mutate('id' = id, 'cluster' = cluster)
