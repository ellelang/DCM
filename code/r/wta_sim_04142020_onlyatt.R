...
#da0<- read.csv (file = "wta_observables11192018.csv",header = TRUE)
#dim(da0)
#names(da0)
#da1 <- left_join(da0, subset, by ='id' )
#names(da1)
#dataset <- da1
...
dataset$lake1000 <- dataset$implakes/1000
colnames(dataset)
betas <- read.table("betas_onlyatt.txt", sep = "\t", header = TRUE)
dim(betas)
varcov <- read.table("varscov_onlyatt.txt", sep = "\t", header = FALSE)
dim(varcov)

varcov_m <- as.matrix(varcov, nrow = 6, ncol = 6)
...
draws <- matrix (NA,nrow = 10, ncol = 6)
colnames(draws)<-c('wetland','pay','cc','nm','asc','clake')
dim(varcov_m)
for (r1 in 1 : 10){
  drawbetas<- betas
...
for (i in 1:s){
  ...
  for (r in 1 : R){
    ...
    new_beta2<- c(0,0,0,0,newbetas[5], 0)
    index_i <- index [i]
    dat_s <- dat0[i,]
    dat_s_m <- as.matrix(x = dat_s, nrow = 1, ncol = 6)  
    #wta_s <- -(wld_rp * rexp(rate = 1, n = n_draws) + asc) / (pay_rp)
...
#   
# write.csv (x = WLD_WTA_ALL, file = "WLD_wta_onlyatt.csv", row.names = FALSE)
# write.csv (x = CC_WTA_ALL, file = "CC_wta_onlyatt.csv", row.names = FALSE)
# write.csv (x = NM_WTA_ALL, file = "NM_wta_onlyatt.csv", row.names = FALSE)

write.csv (x = WLD_WTA_ALL, file = "WLD_wta_onlyattmarginal.csv", row.names = FALSE)
write.csv (x = CC_WTA_ALL, file = "CC_wta_onlyattmarginal.csv", row.names = FALSE)
write.csv (x = NM_WTA_ALL, file = "NM_wta_onlyattmarginal.csv", row.names = FALSE)


wld = read.csv("WLD_wta_onlyatt.csv")
cc = read.csv("CC_wta_onlyatt.csv")
nm = read.csv("NM_wta_onlyatt.csv")
mean(cc$MEAN)
plot(density(cc$MEAN))
