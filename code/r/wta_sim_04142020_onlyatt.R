...
varcov_m <- as.matrix(varcov, nrow = 7, ncol = 7)
...
draws <- matrix (NA,nrow = 10, ncol = 7)
colnames(draws)<-c('wetland','pay','cc','nm','asc','plake','clake')
dim(varcov_m)
for (r1 in 1 : 10){
  drawbetas<- betas
...
for (i in 1:s){
  ...
  for (r in 1 : R){
    ...
    index_i <- index [i]
    dat_s <- dat0[i,]
    dat_s_m <- as.matrix(x = dat_s, nrow = 1, ncol = 7)  
    wta_s <- -(wld_rp * rexp(rate = 1, n = n_draws) + asc) / (pay_rp + lake_p)
...
for (i in 1:s){
  ...
  for (r in 1 : R){
    ...
    lake_p <- newbetas["plake"]
    ## individual-specific parameters
    new_beta2<- c(0,0,0,0,newbetas[5], 0,0)
    index_i <- index[i]
    dat_s <- dat0[i,]
    dat_s_m <- as.matrix(x = dat_s, nrow = 1, ncol = 7)  
